<h1 align="center">
  <img src="https://d1nhio0ox7pgb.cloudfront.net/_img/i_collection_png/256x256/plain/tools.png" alt="AutoTool">
  <br>
  <span style="color: #00ff00; text-shadow: 0 0 10px #00ff00;">AutoTool</span> - Универсальная сборка для работы с Телефонами Redmi A5 / Poco C71
</h1>

<p align="center">
  <img src="https://img.shields.io/badge/Версия-1.1.6-blue?style=flat-square" alt="Версия">
  <img src="https://img.shields.io/badge/Windows-7|10|11-0078D6?style=flat-square&logo=windows" alt="Windows">
  <img src="https://img.shields.io/badge/Архитектура-x64-important?style=flat-square" alt="x64">
  <img src="https://img.shields.io/badge/Язык-Русский-red?style=flat-square" alt="Русский">
  <img src="https://img.shields.io/badge/Режим-PORTABLE-blue?style=flat-square" alt="Portable">
  <img src="https://img.shields.io/badge/Лицензия-GPL_2.0-blue?style=flat-square" alt="Лицензия">
   <img src="https://img.shields.io/badge/Дата_обновления-13.01.2026-blue?style=flat-square" alt="Лицензия">
</p>

<div align="center">
  <h2>🚀 <span style="color: #ff6600;">Представляю вам мою программу AutoTool</span></h2>
  <p>В ней собраны компоненты для телефонов Redmi A5 / Poco C71 </p>
  </p>
  
</p>
   Сборка защищена лицензией General Public License v2 (GPL-2.0).
</p>
  <p>Что нового в v1.0 : 
    <p>
      Новое меню
    <p>
    <p>
      1: Unlock Bootloader
      <p>
<p>Это было сделано, для автоматического разблокировки загрузчика<p>
  <p>
    2: Auto GSI 
    <p>Это было сделано, для автоматического установки GSI прошивки.<p>
        <p>
        3: обновлены составляющие сборки и программы. А также реализована поддержка Windows 7.
        <p>
</div>

## 📦 <span style="color: #00aaff;">Содержимое сборки </span>
- <span style="color: #4CAF50;">**Драйверы**</span> - для всех устройств Unisoc (ADB/Fastboot/Port)
-
## 🎮 **Управление сборкой**
<div style="background: #1a1a1a; padding: 15px; border-radius: 10px; border-left: 5px solid #00ff00;">
<strong>🎯 Основной интерфейс:</strong> <code>AutoTool.exe</code><br>
<span style="color: #cccccc;">Меню на основе Windows CMD с множеством функций автоматизации</span>
</div>

<p align="center">
<a href="https://github.com/deloruzovv/AutoTool/blob/main/AutoTool.exe">
  <img src="https://img.shields.io/badge/Ознакомиться_с_функциями_menu-00aa00?style=for-the-badge" alt="Функции/menu">
</p>

## 💻 **Требования к системе**
<table>
<tr>
  <th align="center">Тип</th>
  <th align="center">Система</th>
  <th align="center">Статус</th>
</tr>
<tr>
  <td align="center">✅ <strong>Рекомендуемые</strong></td>
  <td align="center">Windows 10-11 x64</td>
  <td align="center"><span style="color: #4CAF50;">Идеальная работа</span></td>
</tr>
<tr>
  <td align="center">⚠️ <strong>Минимальные</strong></td>
  <td align="center">Windows 7 x64</td>
  <td align="center"><span style="color: #FF9800;">Возможны ограничения</span></td>
</tr>
<tr>
  <td align="center">❌ <strong>Не рекомендуется</strong></td>
  <td align="center">Windows x86</td>
  <td align="center"><span style="color: #F44336;">множественые ошибки</span></td>
</tr>
</table>

### 🌐 **Язык, кодировка, информация:**
- **Язык интерфейса:** <span style="color: #F44336;">Русский</span>
- **Кодировка:** UTF-8 / UNICODE
- **Режим работы:** PORTABLE
- **Версия:** 1.1.6
- 
- **ВНИМАНИЕ**
- 1) Автор не несёт ответственность за сборку, ваши действия и последствия !
- 2) Окончание (MOD) озночает что программа платная но предоставляется с полным функционалом (PRO) (для активации програмы используется патч )
- 3) Сборка имеет лицензию General Public License v2 (GPL-2.0).
  4) Сборка собрана из ИСХОДНИКОВ и ОТКРЫТОГО исходного кода
  5) Перед установкой, пожалуйста, отключите антивирус и добавьте папку, в которую будет установлена сборка, в исключения антивируса.

## 📥 **Загрузка**
<div align="center" style="margin: 30px 0;">
<a href="">
  <img src="https://img.shields.io/badge/СКAЧАТЬ_1.1.6_822_MB-00ff00?style=for-the-badge&logo=google-drive&logoColor=white" width="300" alt="Скачать">
<a href="https://cloud.mail.ru/public/5tYa/maRBArJ89">
  <img src="https://img.shields.io/badge/СКАЧАТЬ_1.1.6_822_MB-00ff00?style=for-the-badge&logo=mail.ru&logoColor=white" width="300" alt="Скачать">
  <a href="https://github.com/deloruzovv/AutoTool/releases">
  <img src="https://img.shields.io/badge/СКАЧАТЬ_1.1.6_822_MB-00ff00?style=for-the-badge&logo=GitHub&logoColor=white" width="300" alt="Скачать">
</a>
</div>

## 🆘 **Поддержка, обратная связь, предложения**
<div align="center">
<a href="https://github.com/deloruzovv/AutoTool/discussions/">
  <img src="https://img.shields.io/badge/Предложения_и_обсуждения-4285F4?style=for-the-badge&logo=github&logoColor=white" alt="Обсуждение, предложения">
</a>
</div>

